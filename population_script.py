...
os.environ.setdefault('DJANGO_SETTINGS_MODULE',
                      'employees_demo_heroku.settings')
...
def generate_emp(sal_min, sal_max, title_fk, dept_fk):
    ...
    emp = Employees(first_name=fname,
                    ...
                    gender=gend,
                    birth_date=fake.date_of_birth(minimum_age=35,
                                                  maximum_age=65),
...
def assign_all_managers():
    ...
    manager_title_id = (Titles.objects.filter(title="Manager")
                        .values('id')[0]['id'])
    for dept_id in range(1, dept_count + 1):
        mgr_id = (Employees.objects
                  .filter(department_id=dept_id, title=manager_title_id)
                  .values('id')[0]['id'])
        (Employees.objects.filter(department_id=dept_id)
         .update(manager_id=mgr_id))
